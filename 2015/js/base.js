...
$(document).ready(function(){
	...
	// click into a card
	$('#speakers .card').click(function(e){
		e.preventDefault();
		if ( !$(this).is('.blank') ) {
			var theCard = $(this);
	...
	// on mobile, using the dev / design tabs for the agenda
	$('#agenda .mobile_nav a').click(function(e){
		e.preventDefault();
		$(this).parent().addClass('active').siblings().removeClass('active');
		if ( $(this).is('.dev_link') ) {
			$('#agenda').removeClass('show_design').addClass('show_dev');
		}
		else {
			$('#agenda').removeClass('show_dev').addClass('show_design');
		}
	});
		...
		if ( isModalOpen == 'yes' ) {
			...
			setTimeout(function(){
				...
				setTimeout(function(){
					...
					$('#modal').removeClass('show dev design');
					setTimeout(function(){
						$('#modal').fadeOut(50, function(){
